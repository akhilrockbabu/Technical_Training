...
Input Format
The first line of input contains t, 1 st s 5, which gives the number of test cases.
...
In the second test case there are dogs of size 30, 40, 20, 41 and 50, and four dog walkers. There are so many dog walkers that we can ask all but one of them to walk a single dog. We will make the last dog walker walk the dogs of size 40 and 41, which gives a range of
41-40=1. All other groups have range 0, so the total is 1.'''


def dogwalker(n,k,arr):
    arr.sort()
    diff_arr=[arr[i]-arri[i-1] for i in range(1,n)]
    diff_arr.sort(reverse=True)
    return arr[-1]-arr[0]-sum(diff_arr[:k-1])

t=int(input('enter the number of workers'))

while(t):
    n,k=map(int,input().split())
    arr=list(map(int,input().split()))
    print(dogwalker(n,k,arr))
    t-=1
